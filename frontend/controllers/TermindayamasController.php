...
/**
 * TermindayamasController implements the CRUD actions for Termindayamas model.
 */
class TermindayamasController extends Controller
{
    ...
    /**
     * Lists all Termindayamas models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new TermindayamasSearch();
    ...
    /**
     * Displays a single Termindayamas model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new Termindayamas model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate($id)
    {
        $model = new Termindayamas();
        $model->id_daymas= $id;
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($foto1==NULL||$foto2==NULL){
                ...
                return $this->redirect(['/dayamasyarakat/view', 'id' => $model->id_binmas]);
            }
            if($foto1!=NULL){
            ...
            if($model->save()){
                if($foto1 !=NULL){
                    $foto1->saveAs(Yii::getAlias('Upload/').'terminDayaDesa/'.$model->foto_1);
                }
                if($foto2 !=NULL){
                    $foto2->saveAs(Yii::getAlias('Upload/').'terminDayaDesa/'.$model->foto_2);
                }
                if($foto3 !=NULL){
                    $foto3->saveAs(Yii::getAlias('Upload/').'terminDayaDesa/'.$model->foto_3);
                }
                if($foto4 !=NULL){
                    $foto4->saveAs(Yii::getAlias('Upload/').'terminDayaDesa/'.$model->foto_4);
                }
                if($foto5 !=NULL){
                    $foto5->saveAs(Yii::getAlias('Upload/').'terminDayaDesa/'.$model->foto_5);
                }
                return $this->redirect(['/dayamasyarakat/view', 'id' => $model->id_daymas]);
            }
            else{
                Yii::$app->session->setFlash('error','Maaf, ada yang salah dalam pengisian form');
                return $this->redirect(['/dayamasyarakat/view', 'id' => $model->id_daymas]);
            }
            //return $this->redirect(['view', 'id' => $model->id_termindayamas]);
        } else {
    ...
    /**
     * Updates an existing Termindayamas model.
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($model->save()){
                ...
                if($f==1){
                    $file1->saveAs(Yii::getAlias('Upload/').'terminDayaDesa/'.$model->file_1);
                }
                if($g==1){
                    $file2->saveAs(Yii::getAlias('Upload/').'terminDayaDesa/'.$model->file_2);
                }
                if($h==1){
                    $file3->saveAs(Yii::getAlias('Upload/').'terminDayaDesa/'.$model->file_3);
                }
                if($i==1){
                    $file4->saveAs(Yii::getAlias('Upload/').'terminDayaDesa/'.$model->file_4);
                }
                if($j==1){
                    $file5->saveAs(Yii::getAlias('Upload/').'terminDayaDesa/'.$model->file_5);
                }
                return $this->redirect(['view', 'id' => $model->id_termindayamas]);
            }
            else{
    ...
    /**
     * Deletes an existing Termindayamas model.
    ...
    public function actionDownloadfile1($id) {
        $posts= Termindayamas::find()->where(['id_termindayamas'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminDayaDesa/'.$posts->file_1;
        ...
        if (@file_exists($src)) {
           ...
           $message = "File tidak tersedia";
            ...
            return $this->redirect(['Termindayamas/view','id_binmas'=>$id]);
            exit();
        }
    }
        ...
        $posts= Termindayamas::find()->where(['id_termindayamas'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminDayaDesa/'.$posts->file_2;
        ...
        $posts= Termindayamas::find()->where(['id_termindayamas'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminDayaDesa/'.$posts->file_3;
        ...
        $posts= Termindayamas::find()->where(['id_termindayamas'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminDayaDesa/'.$posts->file_4;
        ...
        $posts= Termindayamas::find()->where(['id_termindayamas'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminDayaDesa/'.$posts->file_5;
    ...
    /**
     * Finds the Termindayamas model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Termindayamas the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Termindayamas::findOne($id)) !== null) {
            return $model;
        } else {
