...
/**
 * TerminpembdesController implements the CRUD actions for Terminpembdes model.
 */
class TerminpembdesController extends Controller
{
    ...
    /**
     * Lists all Terminpembdes models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new TerminpembdesSearch();
    ...
    /**
     * Displays a single Terminpembdes model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new Terminpembdes model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate($id)
    {
        $model = new Terminpembdes();
        $model->id_pembdes = $id;
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($foto1==NULL||$foto2==NULL){
                ...
                return $this->redirect(['/pembangunandesa/view', 'id' => $model->id_pembdes]);
            }
            ...
            if($model->save()){
                if($foto1 !=NULL){
                    $foto1->saveAs(Yii::getAlias('Upload/').'terminPembangunanDesa/'.$model->foto_1);
                }
                if($foto2 !=NULL){
                    $foto2->saveAs(Yii::getAlias('Upload/').'terminPembangunanDesa/'.$model->foto_2);
                }
                if($foto3 !=NULL){
                    $foto3->saveAs(Yii::getAlias('Upload/').'terminPembangunanDesa/'.$model->foto_3);
                }
                if($foto4 !=NULL){
                    $foto4->saveAs(Yii::getAlias('Upload/').'terminPembangunanDesa/'.$model->foto_4);
                }
                if($foto5 !=NULL){
                    $foto5->saveAs(Yii::getAlias('Upload/').'terminPembangunanDesa/'.$model->foto_5);
                }
                return $this->redirect(['/pembangunandesa/view', 'id' => $model->id_pembdes]);
            }
            else{
    ...
    /**
     * Updates an existing Terminpembdes model.
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($model->save()){
                ...
                if($f==1){
                    $file1->saveAs(Yii::getAlias('Upload/').'terminPembangunanDesa/'.$model->file_1);
                }
                if($g==1){
                    $file2->saveAs(Yii::getAlias('Upload/').'terminPembangunanDesa/'.$model->file_2);
                }
                if($h==1){
                    $file3->saveAs(Yii::getAlias('Upload/').'terminPembangunanDesa/'.$model->file_3);
                }
                if($i==1){
                    $file4->saveAs(Yii::getAlias('Upload/').'terminPembangunanDesa/'.$model->file_4);
                }
                if($j==1){
                    $file5->saveAs(Yii::getAlias('Upload/').'terminPembangunanDesa/'.$model->file_5);
                }
                return $this->redirect(['view', 'id' => $model->id_terminpembdes]);
            }
            else{
    ...
    /**
     * Deletes an existing Terminpembdes model.
    ...
    /**
     * Finds the Terminpembdes model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Terminpembdes the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Terminpembdes::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionDownloadfile1($id) {
        $posts= Terminbinmas::find()->where(['id_terminpembdes'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminPembangunanDesa/'.$posts->file_1;
    ...
    public function actionDownloadfile2($id) {
        $posts= Terminbinmas::find()->where(['id_terminpembdes'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminPembangunanDesa/'.$posts->file_2;
    ...
    public function actionDownloadfile3($id) {
        $posts= Terminbinmas::find()->where(['id_terminpembdes'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminPembangunanDesa/'.$posts->file_3;
    ...
    public function actionDownloadfile4($id) {
        $posts= Terminbinmas::find()->where(['id_terminpembdes'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminPembangunanDesa/'.$posts->file_4;
    ...
    public function actionDownloadfile5($id) {
        $posts= Terminbinmas::find()->where(['id_terminpembdes'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminPembangunanDesa/'.$posts->file_5;
