...
/**
 * TerminbinmasController implements the CRUD actions for Terminbinmas model.
 */
class TerminbinmasController extends Controller
{
    ...
    /**
     * Lists all Terminbinmas models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new TerminbinmasSearch();
    ...
    /**
     * Displays a single Terminbinmas model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new Terminbinmas model.
    ...
    public function actionCreate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($foto1==NULL||$foto2==NULL){
                ...
                return $this->redirect(['/binamasyarakat/view', 'id' => $model->id_binmas]);
            }
            ...
            if($model->save()){
                if($foto1 !=NULL){
                    $foto1->saveAs(Yii::getAlias('Upload/').'terminBinaDesa/'.$model->foto_1);
                }
                if($foto2 !=NULL){
                    $foto2->saveAs(Yii::getAlias('Upload/').'terminBinaDesa/'.$model->foto_2);
                }
                if($foto3 !=NULL){
                    $foto3->saveAs(Yii::getAlias('Upload/').'terminBinaDesa/'.$model->foto_3);
                }
                if($foto4 !=NULL){
                    $foto4->saveAs(Yii::getAlias('Upload/').'terminBinaDesa/'.$model->foto_4);
                }
                if($foto5 !=NULL){
                    $foto5->saveAs(Yii::getAlias('Upload/').'terminBinaDesa/'.$model->foto_5);
                }
                if($file1 !=NULL){
                    $file1->saveAs(Yii::getAlias('Upload/').'terminBinaDesa/'.$model->file_1);
                }
                if($file2 !=NULL){
                    $file2->saveAs(Yii::getAlias('Upload/').'terminBinaDesa/'.$model->file_2);
                }
                if($file3 !=NULL){
                    $file3->saveAs(Yii::getAlias('Upload/').'terminBinaDesa/'.$model->file_3);
                }
                if($file4 !=NULL){
                    $file4->saveAs(Yii::getAlias('Upload/').'terminBinaDesa/'.$model->file_4);
                }
                if($file5 !=NULL){
                    $file5->saveAs(Yii::getAlias('Upload/').'terminBinaDesa/'.$model->file_5);
                }
                return $this->redirect(['/binamasyarakat/view', 'id' => $model->id_binmas]);
            }
            else{
    ...
    /**
     * Updates an existing Terminbinmas model.
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($model->save()){
                ...
                return $this->redirect(['view', 'id' => $model->id_terminbinmas]);
            }
            else{
    ...
    /**
     * Deletes an existing Terminbinmas model.
    ...
    /**
     * Finds the Terminbinmas model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Terminbinmas the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */


    public function actionDownloadfile1($id) {
        $posts= Terminbinmas::find()->where(['id_terminbinmas'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminBinaDesa/'.$posts->file_1;
    ...
    public function actionDownloadfile2($id) {
        $posts= Terminbinmas::find()->where(['id_terminbinmas'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminBinaDesa/'.$posts->file_2;
    ...
    public function actionDownloadfile5($id) {
        $posts= Terminbinmas::find()->where(['id_terminbinmas'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminBinaDesa/'.$posts->file_5;
    ...
    public function actionDownloadfile3($id) {
        $posts= Terminbinmas::find()->where(['id_terminbinmas'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminBinaDesa/'.$posts->file_3;
    ...
    public function actionDownloadfile4($id) {
        $posts= Terminbinmas::find()->where(['id_terminbinmas'=>$id])->one();
        $src;
        $src = Yii::getAlias('Upload/').'terminBinaDesa/'.$posts->file_4;
    ...
    {
        if (($model = Terminbinmas::findOne($id)) !== null) {
            return $model;
        } else {
